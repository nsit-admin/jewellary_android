...
const Login = () => {
          ...
          try {
            if (res.status !== 200) {
              ...
              if (res.status === 402) {
                ...
                setModalDesc(
                  "Provided phone number is already a customer of GHT, please Sign In to continue"
                );
              }
            } else {
        ...
        .then(async (res) => {
          try {
            if (res.status !== 200) {
              ...
              if (res.status === 404) {
                setModalDesc("Entered OTP is Invalid");
              } else if (res.status === 300) {
                setModalDesc(
                  "You have tried more than allowed limit of entries. Please try a new one"
                );
              } else if (res.status === 201) {
                setModalDesc("OTP expired, please try a new one");
              } else {
                setModalDesc(
              ...
              if (screenType === "SignIn") {
                navigate("/existing-scheme", {
                  state: { mobileNumber: mobileNumber, isStoreLogin: false },
                });
              }
            }
          } catch (err) {
        ...
        try {
          ...
          if (res.status !== 200) {
            ...
            if (screenType === "StoreLogin") {
              navigate("/existing-scheme", {
                state: { mobileNumber: mobileNumber, isStoreLogin: true },
              });
            } else {
              setIsError(false);
              // setMessage(jsonRes.message);
  ...
  const maxLengthCheck = (object, val) => {
    if (object.target.value.length > val) {
      object.target.value = object.target.value.slice(0, val);
    }
  };
  ...
  return (
    <div className="login">
      ...
      <div className="container">
        ...
        />
        <div className="form">
          {(screenType === "SignIn" || screenType === "StoreLogin") && (
            <h3>Guru Hasti Thanga Maaligai</h3>
          )}
          {(screenType === "SignIn" || screenType === "StoreLogin") && (
            <h4>Welcome to Gold Scheme Online Payment System</h4>
          )}
          {/* {screenType === 'SignIn' && !isOtpVerified && <h5>Login</h5>} */}
          {screenType === "SignUpExisting" && (
            <h6>
              Kindly provide registered phone number and last receipt number
            </h6>
          )}
          {screenType === "SignUpNew" && (
            <h6>
              Kindly provide all the below details to complete your new
              registration
            </h6>
          )}
          ...
          <div className="formContainer">
            ...
            {(screenType === "SignUpNew" || screenType === "SignIn") &&
              isOtpSent &&
              !isOtpVerified && (
                <input
                  type="password"
                  onInput={(e) => maxLengthCheck(e, 6)}
                  maxLength={6}
                  value={otp}
                  name="otp"
                  placeholder="otp"
            ...
            {screenType === "SignUpNew" && isOtpVerified && (
              <input
                type="text"
                value={customerName}
                name="Name"
            ...
            {screenType === "SignUpExisting" && !isOtpVerified && (
              <input
                type="text"
                value={receiptNo}
                name="Last Receipt No"
                placeholder="Last Receipt No"
            ...
            {isOtpSent && !isOtpVerified && resendOTPCount < 3 && (
              <div>
                ...
                {resendOTPTimer > 0 && (
                  <span> in {resendOTPTimer} seconds</span>
                )}
              </div>
            )}
              ...
              {screenType === "SignUpNew" && (
                <div>
                  {isOtpVerified && (
                    <button
                      type="button"
                      onClick={isOtpVerified ? onSubmitHandler : otpHandler}>
                      Register
                    </button>
                  )}
                  {!isOtpVerified && (
                    <button
                      type="button"
                      onClick={isOtpVerified ? onSubmitHandler : otpHandler}>
                      {isOtpSent ? "Verify Phone" : "Get OTP"}
                    </button>
                  )}
                </div>
              )}
            ...
            {screenType === "SignIn" && (
              <>
                ...
                <span
                  onClick={() => {
                    screenTypeHandler("SignUpNew");
                  }}>
                  If you are a new customer to GHT and would like to register,
                  please <Link className="signup"> click here </Link>
                </span>
              </>
            )}
            {screenType != "SignIn" && (
