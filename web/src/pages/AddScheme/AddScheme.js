...
const AddScheme = () => {
  ...
  const [instamt, setInstamt] = useState(null);
  ...
  useEffect(() => {
    ...
    if (customer && !isDefaultsSet) {
      ...
      setAddress1(customer.Add1);
      setAddress2(customer.Add2);
      setAddress3(customer.Add3);
      setIsEditable(false);
      ...
      <div className="container">
        ...
        <div className="savingType">
          ...
          <div onChange={(e) => {setChitType(e.target.value)}}>
            <input type="radio" value="2" name="chitType" checked={chitType === "2"} /> Cash
            <input type="radio" value="1" name="chitType" checked={chitType === "1"} /> Metal
          </div>
        </div>
        <form action="">
